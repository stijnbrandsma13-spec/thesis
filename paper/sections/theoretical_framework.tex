...
\end{equation}
We call this the FKRB estimator.\footnote{In the literature, \cref{eq:FKRBCLSest,eq:FKRBest} are interchangeably called the FKRB estimator.}
The original FKRB estimator can be seen as a parametric estimator, since the estimator is determined merely by a $R$-dimensonial parameter vector, with prespecified $R < \infty$.
...
Consider constraints \cref{eq:FKRBconstraint1,eq:FKRBconstraint2}.
...
\end{equation}
This means that under this identification we can rewrite \cref{eq:FKRBconstraint1,eq:FKRBconstraint2} into
\begin{align}
...
\label{eq:FKRBNNLSestpart}
    \begin{split}
        \hat{\bm{\theta}}^{\mathrm{NNL*}} = &\argmin_{\bm{\theta}}{\frac{1}{2NJ}\sum_{i=1}^{N}\sum_{j=1}^{J}(\tilde{y}_{ij}-\tilde{\bm{z}}^\prime_{ij}\bm{\theta})^2}, \\
            &\text{ subject to \cref{eq:FKRBNNLconstraint1,eq:FKRBNNLconstraint2}}.
    \end{split}
\end{equation}
...
\end{equation}
substitute these into \crefrange{eq:FKRBzetavar}{eq:FKRBzetacov} to obtain $\hat{\bm{\Omega}}$, and solve
\begin{equation}
...
Therefore, the difference between these two versions is merely the theoretical paradigm on which these estimators rest.
After the choice of the $R$ grid points the estimators behave exactly the same.
The nonparametric estimator allows for another approach on theoretical asymptotic results.
...
The sieve approach for theoretical results is explored by \citet{FoxKimYang2016}.

\subsubsection{Nonparametric FKRB as PSMD}
We can be precise about the sieve structure by recognizing the nonparametric FKRB estimator as PSMD estimator in the sense of \citet{ChenPouzo2012,ChenPouzo2015}.
Their equivalence will be showed in this section.
This characterization is what will let us bring their inferential theory to FKRB later.

We start by showing that FKRB is an SMD estimator \citep[see][]{AiChen2003}, which is a special case of the PSMD estimator.
...
\end{equation}
where $\bm{\rho} = (\rho_1,\rho_2,\ldots,\rho_J)$ is a $J$-dimensional function resembling generalized residuals and $\alpha_0$ is the (possibly infinite-dimensional) unknown parameter.\footnote{Technically, \citet{ChenPouzo2015} only require that this holds $\bm{x}_i$-almost surely.}
...
On this sieve $\rho_j$ is linear in $\bm{\theta}$:
\begin{equation}
...
\end{equation}
which is exactly the $J\times J$ block of the GLS covariance matrix \cref{eq:FKRBOmega} from \Cref{sec:FKRBGLS}.
...
The feasible GLS estimator $\hat{\bm{\theta}}^{\mathrm{FGLS}}$ of \Cref{sec:FKRBGLS} is therefore exactly the optimally-weighted SMD estimator for this model.

We conclude that the constrained nonparametric FKRB estimator is an SMD estimator in the sense of \citet{ChenPouzo2015}, with sieve $\mathcal{F}_R$, trivial conditional mean estimator, and identity weighting.
...
The M-estimator characterization might allow for an application of \citet{Li2025The,Li2025Inference}, but that is out of the scope of this thesis.
We will rely on the PSMD characterization, since it is the PSMD machinery in \citet{ChenPouzo2015} that delivers the Wald and quasi-likelihood ratio methods we use in \Cref{sec:Inference}.

A central object in that machinery is the sieve variance $||v^*_n||_{\mathrm{sd}}$ of a smooth functional $T:\mathcal{F}\to\mathbb{R}$, which captures the asymptotic spread of the plug-in PSMD estimator $\hat{\eta} \coloneq T(F_{\hat{\bm{\theta}}^{\mathrm{PSMD}}})$.
\citet[][Equation~2.10]{ChenPouzo2015} define a consistent estimator for it by the delta-method sandwich
\begin{equation}
\label{eq:sievevariance}
    \|\hat{v}^*_n\|^2_{\mathrm{sd}} = N\nabla T\!\bigl(\hat{\bm{\theta}}^{\mathrm{PSMD}}\bigr)^\prime\, \hat{\bm{H}}^{-1}\, \hat{\bm{V}}\, \hat{\bm{H}}^{-1}\, \nabla T\!\bigl(\hat{\bm{\theta}}^{\mathrm{PSMD}}\bigr),
\end{equation}
where
\begin{equation}
...
Under the regularity conditions of \citet{ChenPouzo2015}, $\|\hat{v}^*_n\|^2_{\mathrm{sd}}/N$ is a consistent estimator of the asymptotic variance of $\hat{\eta}$; this consistency is what underpins their sieve Wald result in Theorem~4.2 and the sieve QLR result in Theorem~4.3.
We will see in \Cref{sec:FKRBconfidence} that, for the FKRB specialization---linear $\bm{\rho}$, trivial $\hat{\bm{m}}$, and identity weighting---the sandwich \cref{eq:sievevariance} collapses to a cluster-robust delta-method form (cf.\ \cref{eq:CPsievevariance}) that coincides, up to the boundary subtleties discussed there, with the FKRB delta-method variance \cref{eq:FKRBdeltamethod}.
...
Optimally, a confidence interval $\mathrm{CI}_{1-\alpha,\theta}$ would have conservativeness measures $\mathrm{ACD}_{\mathrm{CI}_{1-\alpha,\theta}}(\theta) = \mathrm{RCD}_{\mathrm{CI}_{1-\alpha,\theta}}(\theta) = 0$ for all $\theta$ in the parameter space.
Obviously, the more positive these conservativeness measures are, the more conservative the confidence interval, resulting in a higher probability of a Type II error.
Conversely, the more negative these conservativeness measures are, the more ``aggressive'' these confidence intervals are, resulting in a higher probabilitiy of a Type I error.
In fact, the probability of a Type I error becomes higher than the significance level.\footnote{In this case the so-called ``confidence interval'' is not even a valid confidence interval anymore, because \cref{eq:confidenceinterval} would not hold.}
...
Their method is similar to hypothesis testing based on the Wald's test \citep{Wald1943}.
They construct an initial confidence interval $\mathrm{CI}^{\mathrm{OLS}}_{1-\alpha, \theta_r}$ at the $\alpha$ significance level for $\theta_r$ with heteroskedasticity-robust standard errors that arise from the unconstrained OLS estimator $\hat{\bm{\theta}}^{\mathrm{OLS}}$ from \cref{eq:FKRBOLSest}.
...
The scalar $c_{N,J,R}$ is the standard finite-sample bias correction recommended by \citet[][\S2.3]{CameronMiller2015} (commonly referred to as the CR1 correction):
\begin{equation}
...
Note that the centre point of the interval is determined by the FKRB estimator, whereas the width is determined by the OLS estimator standard errors.
It is also good to note that this method acts as if the estimator is asymptotically normally distributed. 
Although this is wrong at the boundary \citep{Andrews1999}.
Asymptotic normality in the interior has been proven for broad classes of PSMD estimators \citep{AiChen2003,ChenLintonVanKeilegom2003,ChenPouzo2009,ChenPouzo2015}, but the FKRB estimator does not fall in any of the classes for which this has been proved.
However, \citet{FoxKimYang2016} conjecture that asymptotic normality does hold in the interior. 
Of course this allows for confidence intervals that cross outside the $[0,1]$ region, while this is clearly not possible for the real parameters as illustrated by \cref{eq:FKRBconstraint1,eq:FKRBconstraint2}.
...
\end{equation}
The clipping cannot decrease the empirical coverage, while it may reduce the width of the interval.
...
Notice that $\widetilde{\mathrm{CI}}^{\mathrm{FKRB}}_{1-\alpha,\eta} = \mathrm{CI}^{\mathrm{FKRB}}_{1-\alpha,\theta_r}$ when $\eta = T(\bm{\theta})$, where $T: \mathbb{R}^R \to \mathbb{R}$ with $T(\bm{\theta}) = \bm{e}'_r \bm{\theta}$, is the select-$r$ functional.\footnote{Here $\bm{e}_r$ denotes the $r$th standard basis vector.}
...
According to \citet{FoxKimRyanBajari2011}, who cite \citet{AndrewsGuggenberger2010Applications}, $\widetilde{\mathrm{CI}}^{\mathrm{FKRB}}_{1-\alpha,\eta}$ has correct coverage, in the sense that the true coverage is at least $1-\alpha$.
However, they observe that the coverage is abnormally high; the confidence intervals are highly conservative.

\subsubsection{FKRB confidence intervals in the PSMD framework}
The interval $\widetilde{\mathrm{CI}}^{\mathrm{FKRB}}_{1-\alpha,\eta}$ is, up to two small modifications, the sieve Wald confidence interval of \citet[][Theorem~4.2]{ChenPouzo2015} for the functional $\eta = T(F_{\bm{\theta}})$.
...
\end{equation}
where $\hat{\eta} = T(\hat{\bm{\theta}}^{\mathrm{SMD}}) = T(\hat{\bm{\theta}}^{\mathrm{FKRB}})$ is the plug-in PSMD estimator and $\|\hat{v}^*_n\|^2_{\mathrm{sd}}$ is the sieve variance of \citet[][Equation~2.10]{ChenPouzo2015}, see \Cref{eq:sievevariance}.
Specializing \cref{eq:sievevariance} to the FKRB setting amounts to evaluating each of its three ingredients --- the moment-residual sensitivity $\hat{\bm{D}}_i$, the weighting $\hat{\bm{\Sigma}}_i^{-1}$, and the residual $\hat{\bm{m}}(\bm{x}_i, F_{\hat{\bm{\theta}}^{\mathrm{PSMD}}})$ --- at the choices made in \Cref{sec:nonparametricFKRB}, and then substituting back.

Collect the rows $\bm{z}_{ij}^\prime$ of \cref{eq:FKRBzetadef} for $j=1,\ldots,J$ into a per-individual block $\bm{Z}_i \in \mathbb{R}^{J\times R}$; stacking these blocks vertically across $i=1,\ldots,N$ recovers the full $NJ\times R$ design matrix $\bm{Z}$ of \cref{eq:FKRBOLSest}.
...
\end{equation}
The prefactor of $N$ in \cref{eq:sievevariance} is consistent with the sum-not-average convention adopted for $\hat{\bm{H}}$ and $\hat{\bm{V}}$ in \cref{eq:sieveinformation,eq:sievescorevariance}: each $\hat{\bm{H}}^{-1}$ scales as $1/N$ and $\hat{\bm{V}}$ as $N$, so the sandwich $\hat{\bm{H}}^{-1}\hat{\bm{V}}\hat{\bm{H}}^{-1}$ scales as $1/N$, the front $N$ produces $\|\hat{v}_n^*\|^2_{\mathrm{sd}} = O(1)$, and the subsequent division by $N$ delivers the $O(1/N)$ asymptotic variance of $\hat{\eta}$ that \cref{eq:CPsievevariance} expresses.
...
For any support point with $\hat{\theta}_r^{\mathrm{FKRB}}$ strictly in the interior of $[0,1]$ the constrained and unconstrained estimators coincide, so the two variances agree exactly at every interior coordinate and differ only on the boundary; in any case the difference is asymptotically negligible, so this choice has no first-order consequence.
Second, FKRB explicitly intersect the resulting interval with the feasible range of the functional, $[\min_{\bm{\theta}\in\Delta_R} T(\bm{\theta}), \max_{\bm{\theta}\in\Delta_R} T(\bm{\theta})]$, whereas \citet{ChenPouzo2015} do not.
Because the true $\eta_0 = T(F_0)$ lies inside this feasible range by construction, the clip leaves coverage exactly unchanged while (weakly) shortening width.

The two procedures are therefore asymptotically equivalent and, at finite samples, differ only by the coverage-preserving clipping step.
This equivalence is what allows us to import the inferential machinery from \citet{ChenPouzo2015} directly: their Theorem~4.2 establishes the asymptotic validity of \cref{eq:CIChenPouzo}, and hence of $\widetilde{\mathrm{CI}}^{\mathrm{FKRB}}_{1-\alpha,\eta}$.
This is a partial answer to the worry raised below \cref{eq:CIOLS} that the FKRB construction acts ``as if'' the estimator were asymptotically normal: in the interior of $\Delta_R$, the theory of \citet{ChenPouzo2015} validates the implied Gaussian approximation for the plug-in PSMD estimator.
The boundary case in which $F_0$ assigns zero mass to one or more grid points (so $\theta_{0,r} = 0$ for some $r$) is, however, ruled out by \citet[p. 1021]{ChenPouzo2015}, which requires the true parameter to be in the interior of the parameter space.
The boundary breakdown of asymptotic normality identified by \citet{Andrews1999} is therefore not resolved by appealing to \citet{ChenPouzo2015}.
Beyond justifying $\widetilde{\mathrm{CI}}^{\mathrm{FKRB}}_{1-\alpha,\eta}$ in the interior, the connection also opens the door to an alternative procedure from \citet{ChenPouzo2015} — the sieve quasi-likelihood ratio interval, discussed in the next subsection — that no longer requires a separate estimate of the asymptotic variance of $\hat{\eta}$ and respects the feasibility of $\eta$ automatically.

\subsubsection{Sieve QLR confidence intervals}
\label{sec:QLRCI}
Both $\widetilde{\mathrm{CI}}^{\mathrm{FKRB}}_{1-\alpha,\eta}$ and its sieve Wald counterpart $\mathrm{CI}^{\mathrm{CP}}_{1-\alpha,\eta}$ require an explicit estimate of the asymptotic variance of $\hat{\eta}$ — the cluster-robust sandwich \cref{eq:CPsievevariance} — and must be intersected with the feasible range of $T$ in order to respect the simplex constraint.
An alternative construction that avoids both of these requirements is the sieve quasi-likelihood ratio (QLR) interval of \citet[][\S4.3]{ChenPouzo2015}, which inverts a test statistic that, under optimal weighting, is asymptotically pivotal against a $\chi^2_1$ distribution and whose inversion is automatically confined to the feasible range of $T$.
Note that the QLR construction shares the interior regularity assumptions of the Wald construction and therefore does not in itself resolve the Andrews-type breakdown; we revisit its empirical behaviour at boundary parameters in the simulation study.
...
When the criterion uses non-optimal weighting — including the identity weighting $\hat{\bm{\Sigma}} = \bm{I}_J$ of the original FKRB criterion in \cref{eq:FKRBCLSest} — the raw QLR statistic no longer converges to $\chi^2_1$.
...
\label{eq:QLRnonoptimal}
    \|\hat{u}^*_n\|^2 \times \mathrm{QLR}_N(\eta_0) \overset{d}{\longrightarrow} \chi^2_1,
\end{equation}
...
\end{equation}
Intuitively, the numerator is the ``naïve'' identity-weighted variance of $\bm{c}^\prime\hat{\bm{\theta}}^{\mathrm{OLS}}$ (treating the errors as i.i.d.\ unit-variance) and the denominator is the cluster-robust sandwich variance from \cref{eq:CPsievevariance}.
Under correctly specified weighting the two coincide and $\|\hat{u}^*_n\|^2 \to 1$, so \cref{eq:QLRnonoptimal} reduces to \cref{eq:QLRoptimal}.
...
First, the inversion is naturally confined to the feasible range $[\min_{\Delta_R}T, \max_{\Delta_R}T]$, since $\mathrm{QLR}_N(\eta_0)$ is defined only for $\eta_0$ admitting a restricted minimizer in $\Delta_R$; no separate clipping step is required and the resulting interval is automatically a subset of the feasible range.
Second, when the criterion is optimally weighted as in \Cref{sec:FKRBGLS}, the studentization factor $\|\hat{u}^*_n\|^2$ collapses to one asymptotically and can be omitted, simplifying \cref{eq:CIQLR} to $\{\eta_0 : \mathrm{QLR}_N(\eta_0) \leq \chi^2_{1, 1-\alpha}\}$; the construction then requires no explicit variance estimate at all.
