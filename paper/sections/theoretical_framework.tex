...
The mixed logit can be derived from various behavior specifications \citep{Train2009}.
...
In this model the elements of the support are often called ``types'', referring to the different types of individuals in the population.
...
Assuming individuals maximize utility implies
\begin{equation}
\label{eq:mxlydef}
    y_{ij} = \mathds{1}\{u_{ij} > u_{ik}, \text{ for all } j \neq k\} = \prod_{\substack{k=1; \\ k\neq j}}^{J}\mathds{1}\{u_{ij} > u_{ik}\}.
\end{equation}

As a researcher, one often aims to estimate $F_0$ by $\hat{F}_0$, and possibly to conduct inference on this.
...
Both $R$ and $\bm{\beta}^{(r)}$ should not be parameters of the model; they should be prespecified.\footnote{There exists an extension of the estimator where these do not have to be prespecified, but depend on the sample size $N$. This is discussed in \cref{sec:nonparametricFKRB}.} 
...
In practice one often wants to abide by these constraints. 
...
The original FKRB estimator can be seen as a parametric estimator, since the estimator is determined merely by an $R$-dimensional parameter vector, with prespecified $R < \infty$.
...
\label{eq:FKRBNNLSestpart}
    ...
    \end{split}
\end{equation}
This is an NNL estimator with a prespecified tuning parameter.
...
\end{equation}
Since we merely the FKRB estimator, we have that $\hat{\bm{\theta}}^{\mathrm{FKRB}} = \hat{\bm{\theta}}^{\mathrm{NNL}}$.
...
However, if the grid is too fine, the variance of the estimator will explode.
...
Since the grid points are closer to each other in a finer grid, the correlation between the points is also higher.
...
This estimator is because the estimator a sieve estimator \citep[see][]{Chen2007}.
...
\label{sec:Inference}
After estimation, the researcher often wants to conduct inference on $\hat{\bm{\theta}}^{\mathrm{FKRB}}$.
...
We conduct inference on the FKRB estimator with confidence intervals.
...
Note that both the center point and the width of the interval are determined by the OLS estimator and its clustered heteroskedasticity-robust standard errors.
...
\Cref{eq:CIOLS} allows for confidence intervals that cross outside the $[0,1]$ region, while this is not possible for the real parameters as illustrated by \cref{eq:FKRBconstraint1,eq:FKRBconstraint2}.
...
\citet{HeissHetzeneckerOsterhaus2022} show that much fewer grid points are estimated to have a probability mass of zero when regularization is applied.
This results in fewer grid points that are estimated to be on the boundary.
...
\citet{ChenPouzo2015} adopt a similar generalized framework.
...
Therefore both the center and the width of the interval can be determined by the constrained FKRB estimator, in contrast to the FKRB confidence intervals which use the unconstrained OLS estimators for both the center and the width.
...
Where FKRB use the unconstrained OLS estimator as center point, \citet{ChenPouzo2015} allow for the constrained FKRB estimator as center point.
...
A major disadvantage of this interval is that inverting the QLR test can be numerically expensive.
